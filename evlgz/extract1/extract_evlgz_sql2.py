    ...
    try:
        ...
        for loc in potential_sql_locations:
            ...
            if method_declaration_line != -1 and method_name != "Unknown_Method" and method_identifier not in processed_methods:
                # 提取GZDATE和GZCPDM参数
                gzdate_params, gzcpdm_params = extract_field_parameters(method_content)
                results.append({
                    "文件全路径": os.path.relpath(file_path, root_dir), "方法名": method_name,
                    "行号": method_declaration_line, "方法内容": method_content,
                    "GZDATE参数": gzdate_params, "GZCPDM参数": gzcpdm_params
                ...
                if unknown_loc_id not in processed_methods:
                    # 对于未知方法，尝试从SQL片段提取参数
                    gzdate_params, gzcpdm_params = extract_field_parameters(loc["sql_snippet"])
                    results.append({
        ...
        for elem in xml_root.iter():
            if elem.tag.endswith(("select", "insert", "update", "delete", "sql")):
                ...
                if re.search(sql_keyword_pattern_xml, inner_sql_text):
                    ...
                    # 提取GZDATE和GZCPDM参数
                    gzdate_params, gzcpdm_params = extract_field_parameters(inner_sql_text)
...
def extract_field_parameters(content):
    """提取SQL内容中的GZDATE和GZCPDM参数信息
    返回两个字符串，分别表示GZDATE和GZCPDM的参数格式
    """
    # 初始化结果
    gzdate_params = []
    gzcpdm_params = []
    
    # 处理多行SQL和字符串拼接，将内容整理成一行
    content_normalized = re.sub(r'["\']\s*\+\s*["\']', '', content)
    content_normalized = re.sub(r'\s+', ' ', content_normalized)
    
    # 字段 = 常量值 或 字段 = 参数 的模式
    # 例如：GZDATE=? 或 GZCPDM='123'
    equals_pattern_date = re.compile(r'GZDATE\s*=\s*([^,\s\)]+)')
    equals_pattern_cpdm = re.compile(r'GZCPDM\s*=\s*([^,\s\)]+)')
    
    # IN子句模式
    # 例如：GZCPDM in ('111','222') 或 GZCPDM in (select code from xxx)
    in_pattern_date = re.compile(r'GZDATE\s+in\s*\(([^)]+)\)', re.IGNORECASE)
    in_pattern_cpdm = re.compile(r'GZCPDM\s+in\s*\(([^)]+)\)', re.IGNORECASE)
    
    # BETWEEN模式
    # 例如：GZDATE between ? and ?
    between_pattern_date = re.compile(r'GZDATE\s+between\s+([^\s]+)\s+and\s+([^\s\)]+)', re.IGNORECASE)
    between_pattern_cpdm = re.compile(r'GZCPDM\s+between\s+([^\s]+)\s+and\s+([^\s\)]+)', re.IGNORECASE)
    
    # 查找引号内部的SQL语句
    sql_in_quotes = re.findall(r'["\'](.*?(?:GZDATE|GZCPDM).*?)["\']', content_normalized)
    if not sql_in_quotes:
        # 如果没有找到引号中的SQL，就在整个内容中查找
        sql_in_quotes = [content_normalized]
    
    for sql_part in sql_in_quotes:
        # 处理等号模式
        for match in equals_pattern_date.finditer(sql_part):
            param = match.group(1).strip()
            param_text = f"GZDATE = {param}"
            if param_text not in gzdate_params:
                gzdate_params.append(param_text)
                
        for match in equals_pattern_cpdm.finditer(sql_part):
            param = match.group(1).strip()
            param_text = f"GZCPDM = {param}"
            if param_text not in gzcpdm_params:
                gzcpdm_params.append(param_text)
        
        # 处理IN模式
        for match in in_pattern_date.finditer(sql_part):
            in_values = match.group(1).strip()
            param_text = f"GZDATE in ({in_values})"
            if param_text not in gzdate_params:
                gzdate_params.append(param_text)
                
        for match in in_pattern_cpdm.finditer(sql_part):
            in_values = match.group(1).strip()
            param_text = f"GZCPDM in ({in_values})"
            if param_text not in gzcpdm_params:
                gzcpdm_params.append(param_text)
        
        # 处理BETWEEN模式
        for match in between_pattern_date.finditer(sql_part):
            start_val = match.group(1).strip()
            end_val = match.group(2).strip()
            param_text = f"GZDATE between {start_val} and {end_val}"
            if param_text not in gzdate_params:
                gzdate_params.append(param_text)
                
        for match in between_pattern_cpdm.finditer(sql_part):
            start_val = match.group(1).strip()
            end_val = match.group(2).strip()
            param_text = f"GZCPDM between {start_val} and {end_val}"
            if param_text not in gzcpdm_params:
                gzcpdm_params.append(param_text)
    
    # 根据是否找到参数返回结果
    gzdate_params_str = "; ".join(gzdate_params) if gzdate_params else ""
    gzcpdm_params_str = "; ".join(gzcpdm_params) if gzcpdm_params else ""
    
    return gzdate_params_str, gzcpdm_params_str
...
def main(scan_directory_path):
    ...
    try:
        # 处理提取结果，添加GZDATE和GZCPDM参数信息
        for result in all_results:
            if "方法内容" in result:
                gzdate_params, gzcpdm_params = extract_field_parameters(result["方法内容"])
                result["GZDATE参数"] = gzdate_params
                result["GZCPDM参数"] = gzcpdm_params
                
        df = pd.DataFrame(all_results)
        if not df.empty:
            df_output = df.drop_duplicates(subset=["文件全路径", "方法名", "行号"])
            output_columns = ["文件全路径", "方法名", "行号", "方法内容", "GZDATE参数", "GZCPDM参数"]
